...
class CollaborationBasedRecommendations : IRecommendationService
{
    ...
    public async Task<List<Recommendation>> GetRecommendations(HttpContext context)
    {
        try
        {
            ...
            var predictor =
                mlContext.Model.CreatePredictionEngine<IrisData, ClusterPrediction>(model,
        ...
        {
            _logger.LogError("Error in CollaborationBasedRecommendations: {error}", e);
        }
...
{
    [ColumnName("Features")] [VectorType(3)]
